...
> 更新：2026-08-29。权威实现为 `SessionController`、`ApiModels`、`SessionService` 与 `web/worker/protocol.py`。
...
| GET | `/sessions/{sessionId}/history` | 查询只读持久化历史；不恢复 Agent |
...
最终以 `run_status=CANCELLED` 为准。文件副作用不回滚，Worker 和 Conversation 保留。关闭 Session 前若仍有活动 Run，接口返回冲突；正确顺序是 cancel → 等待 CANCELLED → close → 等待 CLOSED。

### 历史

`GET /sessions` 返回摘要数组；`GET /sessions/{id}/history` 返回 `runs + events`。历史接口只用于复盘，不存在“resume/reopen Agent”接口。
...
Spring → Worker：首条为 `start(sessionId, runId, workspace, prompt, attachments, maxSteps)`；后续为 `run(...)`、`approval_response(...)` 或 `cancel_run(...)`。Worker → Spring：先发 Session 级 `ready`，之后所有 Run 消息携带 `sessionId + runId + sequence`；`sequence` 在整个 Worker/Session 生命周期连续递增。

同一 Worker 内只构造一次 Agent。每次 `run` 调用同一个 `Agent.run()`，所以 Conversation 跨 Run 保留；Worker 退出后数据库仅保留展示事实，不支持 P2 恢复。
